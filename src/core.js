...
import { pathToUpdate, zipArrays, update } from './utils'
...
const Fields = (fieldsProps) => {
  const interceptor = fieldsProps.definition?.interceptor
    ? interceptors[fieldsProps.definition?.interceptor]
    : null
  let {
    definition,
    value,
  ...
  function onChange(v) {
    let vv = v
    if (interceptor?.onChange)
      vv = interceptor.onChange(vv, value, definition, context)
    if (vv.then) {
      // contraversial
  ...
  if (interceptor && interceptor.processFields) {
    theFields = interceptor.processFields({
  ...
  if (interceptor && interceptor.fieldsContext) {
    interceptorContext = interceptor.fieldsContext({
      fields: theFields,
      definition,
      value,
      context,
  ...
  function doOnChange(child_k, v) {
    ...
    onChange(newV, definition.constraints || [])
  }
  const renderedFields = theFields.map((child, i) => {
    ...
    if (child.type === 'DefinedField' && child.master_field) {
      ...
      additionalProps.current = child.definitions[
        (value?.[child?.master_field] || {}).value
      ] || { type: 'Fields', fields: [] } // TODO assumption
    } else if (child.impositions) {
      imposed_value =
        child?.impositions?.[(value?.[child.master_field] || {}).value] // TODO assumption
    }
    return (
      <ItemWrapper key={i}>
        <FormComponent
...
Fields.validatorRunner = (definition, value, parentValue, context) => {
  ...
  if (definition.validators) {
    result.__own = validatorsValidatorRunner(
      definition,
      value,
      parentValue,
      context
  ...
  for (const f of definition.fields || []) {
    if (f.k) {
      ...
      result = {
        ...result,
        ...validateDefinition(f, value, parentValue, context)
      }
    }
  }
  return result
...
const ForeignKeyListField = ({
  ...
  const interceptor = definition?.listInterceptor
    ? interceptors[definition?.listInterceptor]
    : null
  // const id = 'id_' + uuidv4()
  // const { label } = definition
  const vvalue = value || []
...
ForeignKeyListField.validatorRunner = (
  definition,
  value,
  parentValue,
  context
...
ForeignKeyListField.validatorChecker = (
  definition,
  error,
  state,
  parentState,
      ...
      <Wrapper
        ...
        addButton={
          <button
            ...
            type='button'
            onClick={(_) =>
              onChange([...vvalue, { ...newValue, uuid: uuidv4() }])
            }
          >
            Add {definition?.addWhat}
          </button>
...
UUIDListField.validatorChecker = (
  definition,
  error,
  state,
  parentState,
...
RecursiveListField.validatorRunner = (
  definition,
  value,
  parentValue,
  context
...
RecursiveListField.validatorChecker = (
  definition,
  error,
  state,
  parentState,
...
const RecursiveField = ({
  ...
  // TODO use sth different than hasOwnProperty
...
RecursiveField.validatorChecker = (
  definition,
  error,
  value,
  parentValue,
...
const CustomDisplay = ({ definition, value, context }) => {
  ...
  if (!Widget) {
    console.warn('missing widget', definition.widget)
    return ''
  }
...
validateDefinition = (definition, state, parentState, context) => {
  ...
  let theFields = definition.fields
  const interceptor = definition?.interceptor
    ? interceptors[definition?.interceptor]
    : null
  if (interceptor && interceptor.processFields) {
    theFields = interceptor.processFields({
      fields: theFields,
...
// Validator checkers
const validatorsValidatorChecker = (
  definition,
  error,
  state,
  parentState,
...
definitionIsInvalid = (definition, error, state, parentState, context) => {
  const validatorChecker =
    formComponents[definition.type].validatorChecker ||
    validatorsValidatorChecker
  let theFields = definition.fields
  const interceptor = definition?.interceptor
    ? interceptors[definition?.interceptor]
    : null
...
const interceptStateChange = _.debounce(
  ({
    oldState,
    newState,
    externalContext,
    ...
    if (intcpt) {
      const newExternalContext = intcpt.getExternalContext({
        state: newState,
        serverContext
      })
      ...
      if (JSON.stringify(externalContext) !== JSON.stringify()) {
        setState((state) =>
          intcpt.updateState({ state: newState, context: newExternalContext })
        )
      }
    }
  },
...
export const GenericForm = (props) => {
  const { serverErrors, data, onChange, externalInterceptor, serverContext } =
    props
  ...
  if (props.submitButtonWidget)
    SubmitButtonWidget = submitButtonWidgets[props.submitButtonWidget]
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {formWideError && (
            <div className='form-row normal-spacing'>
